...
class GtpConnectionGo1(gtp_connection.GtpConnection):
    ...
    def score_cmd(self, args):
        ...
        if len(territoryArray) > 1:
            ...
            # need to check if the area is fully enclosed by one color 
            isBlack = False         # keeps track of if territory belongs black 
            isWhite = False         # keeps track of if territory belongs white
            colorSet = set()        # keeps track of colors in territory boarder
            territorySet = set()    # keeps track of boarder
                
            for point in territoryArray[0]:
                colorSet.add(scoreBoard[point + correctionFactor])
                territorySet.add(point)
            ...
            if (isBlack and not isWhite) or (not isBlack and isWhite):
                # it is all of one color and possible needs to be adjusted 
                size = self.board.size
                boardEnclosed = True    # assume all the points enclose the board 
                                
                # check if it's just an area enclosed by a color
                for num in range(0, size):
                    if num == 0:
                        for point in range(1,size-1):
                            if point not in territorySet:
                                boardEnclosed = False 
                    elif num == size-1:
                        sizeSqrd = size * size
                        for point in range(sizeSqrd, sizeSqrd+(size-2)):
                            if point not in territorySet:
                                boardEnclosed = False
                    else:
                        if (num*(size+1)) not in territorySet:
                            boarderEnclosed = False
                        if (num*(size+1))+(size-1) not in territorySet:
                            boarderEnclosed = False
                    
                
                if isBlack and boardEnclosed:
                    # mark all black 
                    for point in pointArray[0]:
                        scoreBoard[point+correctionFactor] = 7
                    
                elif isWhite and boardEnclosed:
                    # mark all white 
                    for point in pointArray[0]:
                        scoreBoard[point+correctionFactor] = 8
                
                # check if the point is an eye 
                if len(pointArray[0]) == 1:
                    point = pointArray[0].pop()
                    pointArray[0].add(point)
                    if self.board.is_eye(point, 1) == 1:
                        # the eye is black
                        scoreBoard[point+correctionFactor] = 7
                    if self.board.is_eye(point, 2) == 2:
                        # the eye is white 
                        scoreBoard[point+correctionFactor] = 8
    ...
    def _count_stones(self, scoreBoard, correctionFactor):
        ...
        param : scoreBoard: the scoreboard to count 
        param : correctionFactor: amount to adjust index by s.t. it accounts for boundry 
        
        return : whiteCount: white's final score
        return : blackCount: black's final score 
        """
        whiteCount = 0
        blackCount = 0
