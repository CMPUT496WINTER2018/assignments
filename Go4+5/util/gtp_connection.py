...
class GtpConnection():
    ...
    def genmove_cmd(self, args):
        ...
        try:
            ...
            move = self.go_engine.get_move(self.board, GoBoardUtil.color_to_int(board_color))
            if move is None:
                self.respond("pass")
                return
    ...
    def convert_probabilities_to_sims_and_wins(self, prob_list):
        #---------convert probabilities into simulation count and wins---------
        #sim(i):     Pmax -> PRIOR_KNOWLEDGE_STRENGTH = 10 simulations
        #            all other moves -> PRIOR_KNOWLEDGE_STRENGTH * (Pi/Pmax)
        #            int(round(x))
        #winrate(i): 0.5Pi/Pmax + 0.5
        #wins(i):    int(round(winrate(i) * sim(i)))
